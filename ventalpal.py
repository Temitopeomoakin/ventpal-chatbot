...
# ================================ FIXED RAG with Collection Name Handling ================================
@st.cache_resource(show_spinner=False)
def create_vector_store() -> Optional[Chroma]:
    """Create vector store with collection name flexibility like Colab"""
    ...
    try:
        ...
        # EXACT same as your working Colab
        ...
        # Try multiple collection name variations (like your Colab did)
        collection_attempts = [
            COLLECTION_NAME,  # "cbt_docs_fresh_2024" 
            f"my_{COLLECTION_NAME}",  # "my_cbt_docs_fresh_2024" (what worked in Colab)
            "my_cbt_docs_fresh_2024",  # Exact name from Colab
            None  # Try without collection name
        ]
        
        for collection_attempt in collection_attempts:
            try:
                if collection_attempt:
                    st.sidebar.info(f"🔄 Trying collection: {collection_attempt}")
                    vectorstore = Chroma(
                        collection_name=collection_attempt,
                        embedding_function=embeddings,
                        persist_directory=VECTOR_DB_PATH
                    )
                else:
                    st.sidebar.info("🔄 Trying without collection name...")
                    vectorstore = Chroma(
                        embedding_function=embeddings,
                        persist_directory=VECTOR_DB_PATH
                    )
                
                # Test if it works (same test as your Colab)
                ...
                if test_docs:
                    ...
                    if collection_attempt:
                        st.sidebar.info(f"📊 Using collection: {collection_attempt}")
                ...
                if "no such column" not in error_msg.lower():
                    ...
                    st.sidebar.warning(f"⚠️ Failed with '{collection_attempt}': {error_msg[:50]}")
                continue
        
        # If all attempts fail
        st.sidebar.error("❌ Could not load vector store with any collection name variation")
        st.sidebar.info("💡 Try updating your requirements.txt with: langchain==0.1.20")
        return None
